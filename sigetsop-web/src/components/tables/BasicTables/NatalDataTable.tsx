...
import { natalDataService, NatalData } from "../../../services/natalDataService";
import NatalDataModal from "../../ui/modal/NatalDataModal";
...
const columns = [
  { 
    key: "person_associated", 
    label: "Persona Asociada", 
    bold: true,
    formatter: (_: any, record: NatalData) => {
      if (record.relationship_type === "officer") {
        const p = record.personnel_data;
  ...
  { 
    ...
    formatter: (v: string) => v === "officer" ? "Funcionario" : "Pareja Civil" 
  },
...
interface NatalDataTableProps {
  personnelId?: number;
}

export default function NatalDataTable({ personnelId }: NatalDataTableProps) {
  const [data, setData] = useState<NatalData[]>([]);
  const [totalItemsCount, setTotalItemsCount] = useState(0);
  const [loading, setLoading] = useState(true);
  const [showDeleted, setShowDeleted] = useState(false);

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [recordToEdit, setRecordToEdit] = useState<NatalData | null>(null);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let response;
      if (personnelId) {
        response = await natalDataService.getByPersonnel(personnelId, params);
      } else {
        response = await natalDataService.list(params);
      }
      ...
      if (response.data.results) {
        ...
        // Asume no paginado o sub-lista para personnelId
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "No se pudieron cargar los datos natales.",
      });
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchData();
  }, [personnelId, itemsPerPages, currentOffset, showDeleted]);
  ...
  // Nuevo: Obtener los elementos de forma directa ya que la API devuelve la página actual
  ...
  const handleExportCSV = async () => {
    try {
      ...
      const response = await natalDataService.exportCSV(params);
      
      const blob = new Blob([response.data], { type: "text/csv" });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `natal_data_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const params = personnelId ? { personnel_id: personnelId } : {};
      const response = await natalDataService.exportPDF(params);
      
      const blob = new Blob([response.data], { type: "application/pdf" });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `natal_data_${new Date().toISOString().slice(0, 10)}.pdf`;
      a.click();
      window.URL.revokeObjectURL(url);
  ...
  const handleEdit = (record: NatalData) => {
    setRecordToEdit(record);
    setIsModalOpen(true);
  };

  const handleDelete = async (record: NatalData) => {
    ...
    if (result.isConfirmed) {
      try {
        await natalDataService.delete(record.id);
  ...
  const handleModalClose = () => {
    ...
    setRecordToEdit(null);
    fetchData(); // Refrescar al cerrar el modal por si hubo un cambio
  };

  const handleSave = (savedRecord: NatalData) => {
    fetchData(); // Siempre refrescar la tabla para obtener el nuevo estado/posición en la paginación
  };

  const getValue = (obj: any, col: any) => {
  ...
  if (loading)
    return <div className="p-5 dark:text-gray-300">Cargando registros...</div>;
                  ...
                  <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                    {currentItems.length === 0 ? (
                      ...
                      currentItems.map((record) => (
                        <TableRow
                          key={record.id}
                          className={`hover:bg-lime-200 dark:hover:bg-lime-700 ${!record.is_active ? "opacity-50" : ""}`}
                        >
                          {columns.map((col) => (
                            <TableCell
                              key={col.key}
                              className={`border border-lime-700 px-1 py-3 text-gray-500 text-theme-sm border border-gray-100 dark:border-white/[0.05] dark:text-gray-400 whitespace-nowrap ${col.bold ? "font-bold" : ""}`}
                            >
                              {getValue(record, col)}
                            </TableCell>
                          ))}
                ...
                <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between">
                  <div className="pb-3 xl:pb-0">
                    <span className="dark:text-gray-200">
                      Mostrando {totalItemsCount > 0 ? (
                        <>
                          <span className="font-medium text-gray-800 dark:text-white">{startItem}</span> a{" "}
                          <span className="font-medium text-gray-800 dark:text-white">{endItem}</span> de{" "}
                        </>
                      ) : null}
                      <span className="font-medium text-gray-800 dark:text-white">{totalItemsCount}</span> registros
                    </span>
                  </div>
                  {totalPages > 1 && (
