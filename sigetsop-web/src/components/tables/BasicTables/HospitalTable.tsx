...
const columns = [
  { key: "name", label: "Hospital", bold: true },
...
export default function HospitalTable() {
  ...
  const filteredData = useMemo(() => {
    ...
    const prioritizedData = data
      .map((hospital) => {
        ...
        if (hospital.name.toLowerCase().includes(lowerCaseSearch)) {
    ...
    return prioritizedData.map((item) => item.hospital);
  }, [data, searchTerm]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await HospitalService.list();
  ...
  const getValue = (
    hospital: Hospital,
    col: { key: string; getter?: (hospital: Hospital) => string },
  ) => {
    if (col.getter) {
      return col.getter(hospital);
    }

    const path = col.key;
    const parts = path.split(".");
    let acc: any = hospital;
  ...
  const handleCreate = () => {
    setHospitalToEdit(null); // Establecer a NULL para CREAR
    setIsModalOpen(true);
  };
  const handleEdit = (hospital: Hospital) => {
    setHospitalToEdit(hospital); // Establecer la unidad específica para EDITAR
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async (hospitalId: number) => {
    if (
      window.confirm(
        `¿Estás seguro de que deseas eliminar la unidad con ID: ${hospitalId}?`,
      )
    ) {
      try {
        await HospitalService.delete(hospitalId);
        setData(data.filter((hospital) => hospital.id !== hospitalId));
  ...
  const handleModalClose = () => {
    ...
    setHospitalToEdit(null); // Limpiar la unidad al cerrar
  };

  const handleSave = (savedHospital: Hospital) => {
    // Actualizar el estado 'data' de la tabla después de guardar/editar
    if (hospitalToEdit) {
      // Edición
      setData(data.map((h) => (h.id === savedHospital.id ? savedHospital : h)));
    } else {
      // Creación
      setData([savedHospital, ...data]); // Añadir al inicio para que se vea
    }
    // No cerramos el modal aquí, eso lo hace el UnitModal
  };

  if (loading)
    return <div className="p-5 dark:text-gray-300">Cargando hospitales...</div>;
