...
def export_personnel_csv(request):
    ...
    personnel = (
        Personnel.objects.filter(is_active=filter_status)
        .select_related("grade", "current_destination")
        .order_by("last_name", "maternal_name", "first_name")
    )

    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = (
        f'attachment; filename="personal_{datetime.now().strftime("%Y%m%d")}.csv"'
    )
    ...
    for p in personnel:
        writer.writerow(
            [
                p.grade.grade_abbr if p.grade else "",
                p.last_name,
                p.maternal_name,
                p.first_name,
                p.middle_name or "",
                p.identity_card,
                p.age,
                str(p.birthdate) if p.birthdate else "",
                p.genre,
                p.phone or "",
                p.years_age,
                str(p.joining_police) if p.joining_police else "",
                p.scale,
                p.insured_number or "",
                p.current_destination.name if p.current_destination else "",
                p.address or "",
                p.door_number or "",
                p.area or "",
                p.reference or "",
                p.reference_phone or "",
            ]
        )
...
@api_view(["GET"])
@permission_classes([IsAuthenticated, IsAdminOrAuxiliarSIT])
def export_personnel_pdf(request):
    """Exportar todo el personal a PDF"""
    ...
    html_string = render_to_string("personnel_export.html", context)
    html = HTML(string=html_string)
    pdf_file = html.write_pdf()

    response = HttpResponse(pdf_file, content_type="application/pdf")
    response["Content-Disposition"] = (
        f'attachment; filename="personal_{datetime.now().strftime("%Y%m%d")}.pdf"'
    )
    return response
...
@api_view(["GET"])
@permission_classes([IsAuthenticated, IsAdminOrAuxiliarSIT])
def export_personnel_all_json(request):
    """Exportar todo el personal como JSON (para uso del frontend)"""
    filter_status = request.query_params.get("filter_status", "True").lower() == "true"
        ...
        data.append(
            {
                ...
                "age": p.age,
                "birthdate": str(p.birthdate) if p.birthdate else "",
                "genre": p.genre,
                "phone": p.phone or "",
                "years_age": p.years_age,
                "joining_police": str(p.joining_police) if p.joining_police else "",
                "scale": p.scale,
                "insured_number": p.insured_number or "",
                "destination": p.current_destination.name
                if p.current_destination
                else "",
                "address": p.address or "",
                "door_number": p.door_number or "",
                "area": p.area or "",
                "reference": p.reference or "",
                "reference_phone": p.reference_phone or "",
            }
        )
