...
class Personnel(models.Model):
    ...
    @property
    def years_age(self):
        """Calcula la antigüedad en años basándose en la fecha de ingreso a la policía"""
        if not self.joining_police:
            return None
        today = date.today()
        return (
            today.year
            - self.joining_police.year
            - (
                (today.month, today.day)
                < (self.joining_police.month, self.joining_police.day)
            )
        )
